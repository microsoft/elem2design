...
class CrelloHFDSHelper(BaseHFDSHelper):
    ...
    def normalize(self, example: Example) -> Example:
        ...
        for attribute in example:
            if attribute not in output:
                if (
                    attribute in self.features and
                    isinstance(self.features[attribute], ds.Sequence)
                    and isinstance(self.features[attribute].feature, ds.Value)
                    and self.features[attribute].feature.dtype.startswith("float")
                ):
                    vmin, vmax = self.get_clamp_params(attribute)
                    output[attribute] = [
                        clamp(v, vmin=vmin, vmax=vmax) for v in example[attribute]
    ...
    def convert_for_renderer(
        ...
        for key in ["width", "left"]:
            example_new[key] = [
                v * example_new["canvas_width"] for v in example_new[key]
            ]
        for key in ["height", "top"]:
            example_new[key] = [
                v * example_new["canvas_height"] for v in example_new[key]
            ]
